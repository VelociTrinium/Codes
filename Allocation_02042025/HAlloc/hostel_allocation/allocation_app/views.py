...
def preferences_view(request):
    ...
    # Load stored preferences for the logged-in user
    user_preferences = []
    if os.path.exists(preferences_file_path):
        with open(preferences_file_path, 'r') as file:
            preferences_data = json.load(file)
            user_preferences = [pref for pref in preferences_data if pref['id'] == reg_number]
    ...
    if request.method == 'POST':
        ...
        preference_data = {
            'id': student_id,
            'preference_no': len(user_preferences) + 1,  # Increment preference number for this user
            'block': block,
            'room_type': room_type,
            'roommates': roommates
        }
        ...
        # Append the new preference to the preferences list
        user_preferences.append(preference_data)

        # Save preferences to the JSON file
        with open(preferences_file_path, 'r+') as file:
            preferences_data = json.load(file) if os.path.getsize(preferences_file_path) > 0 else []
            preferences_data.append(preference_data)
            file.seek(0)  # Reset file position to the beginning
            json.dump(preferences_data, file, indent=4)

        return redirect('preferences')  # Reload the page to reflect the updates

    return render(request, 'preferences.html', {'reg_number': reg_number, 'user_preferences': user_preferences})
